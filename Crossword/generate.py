...
#    print(self.domains)
  ...
  def revise(self, x, y):
    ...
    if self.crossword.overlaps[x, y]:
      xvals = self.domains[x].copy()
      for xval in xvals:
        valid = False
        yvals = self.domains[y].copy()
        for yval in yvals:
          if xval == yval:
            continue
          if xval[self.crossword.overlaps[x, y][0]] == yval[
  ...
  def ac3(self, arcs=None):
    """
        ...
        """
    queue = arcs
    #adds the constraint arcs in the queue
    if not queue:
      queue = []
  ...
  def consistent(self, assignment):
    ...
    for i in assignment:
      ...
      for neighbour in self.crossword.neighbors(i):
        overlap = self.crossword.overlaps[i, neighbour]
        if neighbour not in assignment:    
          continue
        if assignment[i][overlap[0]] != assignment[neighbour][overlap[1]]:
          return False
    return True
  ...
  def order_domain_values(self, var, assignment):
    ...
    for value in self.domains[var]:
      domain_value.append(value)
    return sorted(domain_value, key=lambda val: constraint(var, val))
  ...
  def select_unassigned_variable(self, assignment):
    ...
    unassigned_variables = sorted(
        unassigned_variables,
        key=lambda variable: self.crossword.neighbors(variable),
        reverse=True)
    unassigned_variables = sorted(
        unassigned_variables, key=lambda variable: len(self.domains[variable]))
